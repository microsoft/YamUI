...
const getClasses = (isHeader: boolean) => {
  const classes = ['y-menu-text-button--item'];
  if (isHeader) {
    classes.push('y-menu-text-button--item-header');
  }
  return join(classes);
};

const getIconClasses = (isDisabled: boolean) => {
  const classes = ['y-menu-text-button--item-icon'];
  if (isDisabled) {
    classes.push('y-menu-text-button--item-icon__disabled');
  }
  return join(classes);
};
...
const MenuTextButtonItem: React.StatelessComponent<IContextualMenuItemProps> = (props: IContextualMenuItemProps) => {
  ...
  return (
    <Block textColor={color} textSize={size} className={getClasses(isHeader)}>
      <FixedGridRow gutterSize={GutterSize.SMALL}>
        ...
        <FixedGridColumn className="y-menu-text-button--item-text">{name}</FixedGridColumn>
      </FixedGridRow>
    </Block>
  );
};

export default MenuTextButtonItem;
